    ...
    <Card>
      ...
      <CardContent>
        {isLoading ? <p>Loading…</p> : results.length === 0 ? <p className="text-muted-foreground py-6 text-center">No results yet.</p> : (
          <Table>
            <TableHeader><TableRow><TableHead>Date</TableHead><TableHead>Tournament</TableHead><TableHead>Course</TableHead><TableHead>Score</TableHead><TableHead>Finish</TableHead><TableHead>Field</TableHead>{canEdit && <TableHead></TableHead>}</TableRow></TableHeader>
            <TableBody>
              {results.map((r) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell>{r.field_size || '—'}</TableCell>
                  {canEdit && (
